...
pub fn rgba8_overlay_final(dst_pixels: &mut [Rgba<u8>], src_pixels: &[Rgba<u8>]) -> usize {
        ...
        .zip(src_pixels.chunks(CHUNK_LEN))
    {
        ...
        let dst = unsafe { _mm_loadu_si128(dst_chunk.as_ptr().cast()) };
        let src = unsafe { _mm_loadu_si128(src_chunk.as_ptr().cast()) };
        ...
        unsafe {
            _mm_storeu_si128(dst_chunk.as_mut_ptr().cast(), out);
        }
    }
...
pub fn rgba8_multiply_overlay_final(dst_pixels: &mut [Rgba<u8>], multiply: &Rgb<u8>, src_pixels: &[Rgba<u8>]) -> usize {
    ...
    #[target_feature(enable = "sse4.2")]
    #[inline]
    fn u16x16_rgba_multiply_overlay_final(dst: __m128i, src: __m128i, multiply: __m128i, alpha: __m128i) -> __m128i {
        // src_rgb = src_rgb * multiply
        let src = u16x16_div_by_255(_mm_mullo_epi16(src, multiply));
        // src_rgb * src_a
    ...
    let alpha_mask = _mm_set1_epi32(0xFF000000u32 as i32);
    let zero = _mm_setzero_si128();
    let multiply = multiply.to_rgba();
    let multiply = unsafe { _mm_loadu_si32(multiply.as_ptr().cast()) };
    let multiply = _mm_shuffle_epi32::<0b00000000>(multiply);
    let multiply = _mm_unpacklo_epi8(multiply, zero);
        ...
        .zip(src_pixels.chunks(CHUNK_LEN))
    {
        ...
        let src_a = _mm_shuffle_epi8(src, alpha_shuffle);
        // Process low and high halves upcast from u8 to u16
        let out_lo = u16x16_rgba_multiply_overlay_final(
            _mm_unpacklo_epi8(dst, zero),
            _mm_unpacklo_epi8(src, zero),
            multiply,
            _mm_unpacklo_epi8(src_a, zero),
        );
        let out_hi = u16x16_rgba_multiply_overlay_final(
            _mm_unpackhi_epi8(dst, zero),
            _mm_unpackhi_epi8(src, zero),
            multiply,
            _mm_unpackhi_epi8(src_a, zero),
...
pub fn rgba8_onto_rgb8_overlay(dst_pixels: &mut [Rgb<u8>], src_pixels: &[Rgba<u8>]) -> usize {
        ...
        .zip(src_pixels.chunks(CHUNK_LEN))
    {
        ...
        dst_buf[..12].copy_from_slice(Rgb::<u8>::channels_from_slice(PrivateToken, dst_chunk));
        ...
        Rgb::<u8>::channels_from_slice_mut(PrivateToken, dst_chunk).copy_from_slice(&dst_buf[..12]);
    }
